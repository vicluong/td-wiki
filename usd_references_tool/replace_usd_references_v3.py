...
def checkMissingReferences(main_path):
    stage = getFirstStage()
    if stage:
        stage.Reload()
        ...
        for prim in stage.Traverse():
            ...
            if refs:
                ...
                if missing:
                    prims_with_missing_references.append(prim.GetPath())
                    print("Missing reference found in prim:", prim.GetPath())

        return prims_with_missing_references 
    else:
...
def addReference(prim_path, main_path, file_path):
    stage = getFirstStage()
    root = stage.GetRootLayer()
    stage.SetEditTarget(root)
    ...
    if file_path.startswith(main_path):
...
class CheckReferencesDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent=maya_main_window()):
        ...
        self.resize(int(screen_w * 0.32), int(screen_h * 0.5))
    ...
    def get_missing_references(self):
        if self.main_path_le.text() and os.path.isdir(self.main_path_le.text()):
            prim_paths = checkMissingReferences(self.main_path_le.text())
            ...
            for prim_path in prim_paths :
                ...
                self.problems_table.setCellWidget(index, 1, ref_path_le)
                ...
                apply_btn.clicked.connect(
                    partial(self.fix_ref_path, prim_path, ref_path_le)
                )
    ...
    def fix_ref_path(self, prim_path, widget):
        ...
        if not file_path or not os.path.isfile(file_path):
            ...
            addReference(prim_path, self.main_path_le.text(), file_path)
